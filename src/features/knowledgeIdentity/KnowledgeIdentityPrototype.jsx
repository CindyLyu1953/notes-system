...
function ReviewPanel({ run, busy, onAccept, onReject }) {
  ...
  const states = pending.filter((action) => action.kind === "set_knowledge_state");
  const relations = pending.filter((action) => action.kind === "upsert_relation");
  const stateByConcept = Object.fromEntries(states.map((action) => [action.payload.concept_id, action.payload.state]));
  return <section className="review-panel" aria-live="polite"><div className="review-panel__intro"><span className="eyebrow">Review before organizing</span><h2>Here is how AI understood your input.</h2><p>Your original input is already preserved. Accept these proposed concepts and connections, or reject them without losing the source.</p></div><div className="review-panel__groups"><div><small>Concepts</small>{concepts.map((action) => <span className="review-chip" key={action.id}>{action.payload.name}<b>{stateByConcept[action.payload.concept_id] || "aware"}</b></span>)}</div><div><small>Connections</small><strong>{relations.length}</strong><span>proposed relation{relations.length === 1 ? "" : "s"}</span></div><div><small>Trace</small><strong>{pending.length}</strong><span>evidence-backed actions</span></div></div><div className="review-panel__actions"><button className="review-panel__reject" type="button" onClick={onReject} disabled={busy}>Keep input only</button><button className="review-panel__accept" type="button" onClick={onAccept} disabled={busy}><Icon name="spark" size={18}/>{busy ? "Applying…" : "Accept and organize"}</button></div></section>;
}
...
export default function KnowledgeIdentityPrototype() {
  ...
  const acceptRun = useCallback(async () => { if (!pendingRun) return; setDecisionBusy(true); try { if (pendingRun.demoIdentity) { setIdentity(pendingRun.demoIdentity); } else { setIdentity(await identityApi.commitWorkflow(pendingRun.id)); setMode("api"); } setPendingRun(null); } finally { setDecisionBusy(false); } }, [pendingRun]);
