...
function InputEditor({ item, onSave, busy }) {
  const [content, setContent] = useState(item.content);
  const [destination, setDestination] = useState(item.destination || "");
  return <details className="library-item"><summary><span>{item.destination || item.source_type}</span><small>{item.content}</small></summary><form onSubmit={(event) => { event.preventDefault(); onSave(item.id, { content, destination: destination || null }); }}><label>Title<input value={destination} onChange={(event) => setDestination(event.target.value)} placeholder="Optional"/></label><label>Original input<textarea value={content} onChange={(event) => setContent(event.target.value)}/></label><div className="library-item__meta"><Evidence ids={[item.id]}/><button type="submit" disabled={busy || content.trim().length < 3}>Save</button></div></form></details>;
}
...
function NoteEditor({ item, onSave, busy }) {
  const [title, setTitle] = useState(item.title);
  const [summary, setSummary] = useState(item.summary);
  return <details className="library-item"><summary><span>{item.title}</span><small>{item.summary}</small></summary><form onSubmit={(event) => { event.preventDefault(); onSave(item.id, { title, summary }); }}><label>Title<input value={title} onChange={(event) => setTitle(event.target.value)}/></label><label>Summary<textarea value={summary} onChange={(event) => setSummary(event.target.value)}/></label><div className="library-item__meta"><Evidence ids={item.evidence_input_ids}/><button type="submit" disabled={busy || !title.trim() || !summary.trim()}>Save</button></div></form></details>;
}

function LibraryPage({ identity, busy, notice, onUpdateInput, onUpdateConcept, onUpdateNote, onMerge, onUndo, onExport, onRestore }) {
