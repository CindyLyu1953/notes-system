...
function Graph({ identity }) {
  const conceptById = Object.fromEntries(identity.concepts.map((item) => [item.id, item]));
  const relationLabel = { related_to: "Related to", part_of: "Part of", depends_on: "Depends on", supports: "Supports", contrasts_with: "Contrasts with", applies: "Applies to" };
  if (!identity.concepts.length) return <div className="knowledge-graph knowledge-graph--empty" aria-label="Knowledge concept network"><div className="graph-empty"><Icon name="graph" size={28}/><strong>Your map starts empty</strong><span>Add something above, then approve the concepts that feel true.</span></div></div>;
  return <div className="knowledge-map"><div className="knowledge-map__concepts" aria-label="Knowledge concepts">{identity.concepts.map((concept) => <article className={`map-concept map-concept--${concept.state}`} key={concept.id} title={concept.evidence[0]?.excerpt}><strong>{concept.name}</strong><small>{stateLabel[concept.state]}</small></article>)}</div><div className="graph-connections"><div className="graph-connections__title"><strong>Connections</strong><small>{identity.relations.length} evidence-backed</small></div>{identity.relations.length ? identity.relations.map((relation) => <article className="graph-connection" key={`${relation.source}-${relation.type}-${relation.target}`}><div><strong>{conceptById[relation.source]?.name || relation.source}</strong><span>{relationLabel[relation.type] || "Related to"}</span><strong>{conceptById[relation.target]?.name || relation.target}</strong></div><p>{relation.rationale}</p><small>{Math.round((relation.confidence || 0) * 100)}% confidence · {relation.evidence_input_ids.length} source{relation.evidence_input_ids.length === 1 ? "" : "s"}</small></article>) : <p className="empty-state">No explicit relationships have enough evidence yet.</p>}</div></div>;
}
...
function IdentityPage({ identity, onChat, messages, chatBusy }) {
  return <section className="app-page"><PageIntro title="Knowledge" guide="Ask a question, or open a concept to see the evidence behind it."/><section className="dashboard dashboard--identity"><div className="dashboard__main"><section className="panel identity-panel"><div className="section-title"><span><Icon name="graph"/>Map</span><small>{identity.concepts.length} concepts · {identity.relations.length} connections</small></div><Graph identity={identity}/></section></div><aside className="dashboard__side"><section className="panel"><div className="section-title"><span>Concepts</span></div><ConceptList concepts={identity.concepts}/></section><Chat onChat={onChat} messages={messages} busy={chatBusy}/></aside></section></section>;
}

function NextPage({ recommendation }) {
  return <section className="app-page app-page--focused"><PageIntro title="Next" guide="One useful next step based on what you already know."/><Recommendation item={recommendation}/></section>;
}
...
function SourceRecord({ item, note }) {
  ...
  return <details className="source-record"><summary><span><strong>{title}</strong><small>{new Date(item.created_at).toLocaleDateString()}</small></span><Evidence ids={[item.id]}/></summary><div className="source-record__body"><div className="source-tabs" role="tablist" aria-label={`Views for ${title}`}><button type="button" role="tab" aria-selected={view === "original"} className={view === "original" ? "is-active" : ""} onClick={() => setView("original")}>Original</button><button type="button" role="tab" aria-selected={view === "note"} className={view === "note" ? "is-active" : ""} onClick={() => setView("note")}>AI note</button></div>{view === "original" ? <section className="source-view" role="tabpanel"><div className="source-view__header"><div><strong>Original source</strong><small>Stored unchanged</small></div>{item.attachments?.map((attachment) => attachment.artifact_id ? <a key={attachment.artifact_id} href={artifactContentUrl(attachment.artifact_id)} target="_blank" rel="noreferrer">Open {attachment.name}</a> : null)}</div><pre>{originalText}</pre></section> : <section className="source-view" role="tabpanel"><div className="source-view__header"><div><strong>{note?.title || "Structured note"}</strong><small>AI-organized · concepts highlighted</small></div></div><StructuredNote note={note}/></section>}</div></details>;
}

function ConceptEditor({ item, onSave, busy }) {
  const [name, setName] = useState(item.name);
  const [state, setState] = useState(item.state);
  return <details className="library-item"><summary><span>{item.name}</span><Status state={item.state}/></summary><form onSubmit={(event) => { event.preventDefault(); onSave(item.id, { name, state }); }}><label>Name<input value={name} onChange={(event) => setName(event.target.value)}/></label><label>Knowledge state<select value={state} onChange={(event) => setState(event.target.value)}><option value="unknown">Unknown</option><option value="aware">Aware</option><option value="learning">Learning</option><option value="known">Known</option></select></label><div className="library-evidence">{item.evidence.map((entry) => <p key={`${entry.input_id}-${entry.segment_id || "input"}`}>{entry.excerpt}</p>)}</div><div className="library-item__meta"><Evidence ids={item.evidence.map((entry) => entry.input_id)}/><button type="submit" disabled={busy || !name.trim()}>Save</button></div></form></details>;
}

function LibraryPage({ identity, busy, notice, onUpdateConcept, onMerge, onUndo, onExport, onRestore }) {
  ...
  return <section className="app-page"><PageIntro title="Library" guide="Open a source, then switch between the unchanged original and the structured AI note."/><div className="library-toolbar"><button type="button" onClick={onExport}>Download backup</button><button type="button" onClick={() => importRef.current?.click()}>Restore backup</button><button type="button" onClick={onUndo} disabled={busy}>Undo last change</button><input ref={importRef} className="sr-only" type="file" accept="application/json" onChange={importBackup}/></div>{notice ? <p className="library-notice" role="status">{notice}</p> : null}<div className="library-layout"><section className="panel library-sources"><div className="section-title"><span>Sources</span><small>{identity.recent_inputs.length}</small></div>{identity.recent_inputs.length ? identity.recent_inputs.map((item) => <SourceRecord key={item.id} item={item} note={noteByInput[item.id]}/>) : <p className="empty-state">Nothing captured yet.</p>}</section><section className="panel library-concepts"><div className="section-title"><span>Concepts</span><small>{identity.concepts.length}</small></div>{identity.concepts.length ? identity.concepts.map((item) => <ConceptEditor key={item.id} item={item} onSave={onUpdateConcept} busy={busy}/>) : <p className="empty-state">No concepts yet.</p>}{identity.concepts.length > 1 ? <form className="merge-form" onSubmit={(event) => { event.preventDefault(); if (source && target && source !== target) onMerge(source, target); }}><h3>Merge duplicates</h3><p>Move the first concept and its evidence into the second.</p><select value={source} onChange={(event) => setSource(event.target.value)} aria-label="Concept to merge"><option value="">Merge…</option>{identity.concepts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={target} onChange={(event) => setTarget(event.target.value)} aria-label="Concept to keep"><option value="">Into…</option>{identity.concepts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button type="submit" disabled={busy || !source || !target || source === target}>Merge</button></form> : null}</section></div></section>;
}
